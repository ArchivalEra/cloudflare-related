...
| `max_h2_streams` | 单连接并发流，默认 1（pingap 可配 100 做多路复用）；**进 hash** |
| `h2_stream/connection_window_size` | H2 流/连接级窗口，默认 8MB；**明确排除出 hash** |
...
| `curves` / `second_keyshare` | TLS 曲线/二轮 keyshare；**进 hash** |
