...
| 0 | `init_upstream_modules`（需 feature `upstream_modules`；**main-only，0.8.1 无**） | 启动前注册上游 module，`order()` 大者先跑 | 无返回 |
| 1 | `new_ctx` **必需** | 每请求最先，建 per-request 状态 | 返回 `CTX`，供 `upstream_peer` 等选址用 |
| 1b | `on_connection_reuse` / `persist_connection_context`（**main-only，0.8.1 无**） | 仅 H1 keepalive 复用：前者在 `early_*` 前注入上次状态，后者在 `logging` 后保存 | 默认空 |
...
| 5 | 缓存准入组（详见 `pingora-cache.md`；phase.md 该节标 To be documented，顺序以 trait 源码为准） | `request_filter` 后、`proxy_upstream` 前 | `cache_key_callback` 默认 panic，启用缓存必须重写 |
...
| 12 | `adjust_upstream_modules`（feature `upstream_modules` 门控；**main-only，0.8.1 无此方法**） | 上游头到达、上游 module 之前；1xx 可调多次 | 只读头，改头用 `upstream_response_filter` |
...
| ⊥ | `suppress_error_log` / `suppress_proxy_warn_log`（实验，**后者 main-only**）/ `request_summary` | 正交：压 `fail_to_proxy` 终错误 log / 压可重试上游失败 warn / 返回请求摘要串 | 实现须廉价；压掉 retry warn 会丢逐跳审计，需自补 metrics |
